...
class TriplesGenerator:
	def __init__(self):
		...
		self.data_extracted_dir = '../../data/processed/extracted_triples/'
	...
	def loadData(self):
		for filename in tqdm(list(sorted(os.listdir(self.data_extracted_dir)))[:2]):
			#c = 0
			if filename[-6:] == '.jsonl':
	...
	def run(self):
		...
		print('>> Mapping to external resources - skiped')
		if ckpts_mapping:
			print('\t>> Loaded from ckpts')
			...
			cut_freq = 1
			mapper = EntitiesMapper(self.entitiesFreq(cut_freq), list(all_pairs)) ###
			mapper.run()
